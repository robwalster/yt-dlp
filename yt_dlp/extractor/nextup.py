...
class ViewLiftBaseIE(InfoExtractor):
    ...
    def _fetch_token(self, site, url):
        ...
        cookies = self._get_cookies('https://site-next-up.viewlift.com')
    ...
    def _call_api(self, site, path, video_id, url, query):
        ...
        print("APII DETAILS")
        print("url", self._API_BASE + path)
        print('video_id', video_id)
        print('headers', 'Authorization', self._TOKENS.get(site))
        print('query', query, "\n\n\n")
...
#     _VALID_URL = r'https?://(?:www\.)?site-next-up\.viewliftxxxx\.com/videos/(?P<id>.*)'
...
#         print(self._match_valid_url(url).groups())
#         domain, film_id = self._match_valid_url(url).groups()
#         domain, film_id = ['site-next-up.viewlift.com', '07650ec1-feab-4ae8-bf8b-f4893881acfd']
#         site = 'site-next-up.viewlift'
...
class ViewLiftEmbedIE(ViewLiftBaseIE):
    IE_NAME = 'viewlift:embed'
    _VALID_URL = r'https?://(?:www\.)?site-next-up\.viewlift\.com/embed/(?P<id>.*)'
    ...
    def _real_extract(self, url):
        domain, film_id = 'site-next-up\.viewlift\.com', url.split('=')[1]
        site = domain.split('.')[-2]
        if site in self._SITE_MAP:
            site = self._SITE_MAP[site]
...
class ViewLiftIE(ViewLiftBaseIE):
    ...
    _VALID_URL = r'https?://(?:www\.)?site-next-up\.viewlift\.com/videos/(?P<id>.*)'
    _TESTS = [{
    ...
    def _real_extract(self, url):
        ...
        domain, path, display_id = 'next-up', '/videos/andyfielsfbacky1', 'andyfielsfbacky1'
        # site = domain.split('.')[-2]
        site = 'next-up'
        urla = 'https://prod-api-cached-2.viewlift.com/content/pages'
        print('site:', 'site-next-up.viewlift')
        if site in self._SITE_MAP:
            site = self._SITE_MAP[site]
        modules = self._call_api(
            site, 'content/pages', display_id, urla, {
                'includeContent': 'true',
                'moduleOffset': 1,
                'path': '/videos/archiemasfangel1',
                'site': site,
            })['modules']
        ...
        return {
            ...
            'url': 'http://%s/embed/player?filmId=%s' % ('site-next-up.viewlift.com', film_id),
            'id': film_id,
